...
public class WebBase {
	...
	public static void launchApp() {
		...
		caps.setCapability(MobileCapabilityType.PLATFORM_VERSION, "18.4");
		caps.setCapability(MobileCapabilityType.DEVICE_NAME, "iPad Pro 11-inch (M4)");
		caps.setCapability(MobileCapabilityType.UDID, "3EB6C7E9-D872-4BB0-BC6F-2C79E487258C");
		caps.setCapability(MobileCapabilityType.APP, "Users/dilip/Downloads/EasyEatPartner.app"); 
...
//	}
	public void clickOnElement(By locator, String elementName, long waitTime) {
	    try {
	        ...
	        System.out.println("Unable to click on: " + elementName);
	        e.printStackTrace();
	        throw new AssertionError("Test failed while clicking on: " + elementName, e);
	    }
	}


	
	public void clickOnElement1(By locator, Optional<By> optionalLocator, String elementName, long waitTime) {
        try {
            By finalLocator = optionalLocator.orElse(locator);
            if (driver1 instanceof IOSDriver) {
                WebDriverWait wait = new WebDriverWait(driver1, waitTime);
                MobileElement eleClick = (MobileElement) wait.until(ExpectedConditions.elementToBeClickable(finalLocator));
                eleClick.click();
            } else {
                WebDriverWait wait = new WebDriverWait(driver, waitTime);
                WebElement eleClick = wait.until(ExpectedConditions.elementToBeClickable(finalLocator));
                eleClick.click();
            }
            System.out.println("Successfully clicked on: " + elementName);
        } catch (Exception e) {
